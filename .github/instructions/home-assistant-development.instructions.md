...
- Use SemVer base versions with PEP 440 prereleases. While this project is below `1.0`, incompatible contract changes and compatible features increment minor, while compatible fixes increment patch. Preview versions use monotonically increasing `X.Y.ZbN`; use `X.Y.ZrcN` only after feature completion. Tags are immutable and exactly `v<manifest-version>`.
