...
- Use SemVer base versions with PEP 440 prereleases. While this project is below `1.0`, incompatible contract changes and compatible features increment minor, while compatible fixes increment patch. Both `X.Y.ZbN` and `X.Y.ZrcN` are valid preview channels: use beta while feedback may change behavior or scope, and RC when the owner deliberately declares a release candidate whose intended scope is complete and final or manual validation remains. Honor an explicit owner choice and never silently switch channels; after publishing an RC for a base version, do not move that base version back to beta. Tags are immutable and exactly `v<manifest-version>`.
